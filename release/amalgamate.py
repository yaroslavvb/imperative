...
# fork of 0.1.1 from https://github.com/xonsh/amalgamate/
# modifications are marked with "NOTE/ENDNOTE"
...
def main(args=None):
    ...
    for pkg in args[1:]:
        if pkg.startswith('--debug='):
            debug = pkg[8:]
            continue
        print('Amalgamating ' + pkg + ' from ' + os.getcwd())
        exclude = read_exclude(pkg)

        # NOTE(yaroslavvb): imperative-specific modification
        exclude = {"env_cache_test", "env_test", "itensor_test", "lbfgs_test",
                   "mnist_inference_test", "module_rewriter_test",
                   "power_benchmark", "tensor_handle_test",
                   "test_util", "imperative"}
        # ENDNOTE
